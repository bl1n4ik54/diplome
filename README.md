...
| --- | --- |
| Главная страница с блоками «Продолжить чтение», «В тренде», «Новинки» | Dashboard со сводкой по пользователям, тайтлам, главам и оценкам |
...
| Рейтинги и персональные списки чтения | Защита `/admin` и `/api/admin` через middleware |
...
| `user` | Всё выше, а также профиль, списки чтения, прогресс и друзья |
...
3. Создай базу данных и примени SQL-миграции из папки `drizzle/` в порядке номеров: от `0000_...sql` до `0007_...sql`.
...
`users`, `authors`, `genres`, `comics`, `comic_genres`, `chapters`, `chapter_pages`, `covers`, `ratings`, `user_comic_lists`, `friend_requests`, `reading_progress`.
